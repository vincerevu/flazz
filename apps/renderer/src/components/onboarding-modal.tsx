...
export function OnboardingModal({ open, onComplete }: OnboardingModalProps) {
  ...
  */
  ...
  const renderLlmSetupStep = () => {
    const primaryProviders: Array<{ id: LlmProviderFlavor; name: string; icon: string; description: string; tag?: string }> = [
      { id: "openai", name: "OpenAI", icon: "openai", description: "GPT models for fast, capable general AI tasks" },
      { id: "anthropic", name: "Anthropic", icon: "anthropic", description: "Direct access to Claude models, including Pro and Max" },
      { id: "google", name: "Google", icon: "google", description: "Gemini models for fast, structured responses" },
      { id: "openrouter", name: "OpenRouter", icon: "openrouter", description: "Claude, GPT, Gemini and more from a single key" },
    ]

    const moreProviders: Array<{ id: LlmProviderFlavor; name: string; icon: string; description: string; tag?: string }> = [
      { id: "aigateway", name: "Vercel AI Gateway", icon: "vercel", description: "Route requests through Vercel's AI Gateway" },
      { id: "ollama", name: "Ollama", icon: "synthetic", description: "Run local models on your machine", tag: "Local" },
      { id: "openai-compatible", name: "Custom", icon: "synthetic", description: "Use any OpenAI-compatible endpoint", tag: "Custom" },
    ]

    const showcaseOnly = [
      { name: "OpenCode Zen", icon: "opencode", description: "Reliable optimized models", tag: "Recommended" },
      { name: "OpenCode Go", icon: "opencode-go", description: "Low cost subscription for everyone", tag: "Recommended" },
      { name: "GitHub Copilot", icon: "github-copilot", description: "AI models for coding assistance via GitHub Copilot" },
    ]
    ...
    const renderProviderCard = (provider: { id: LlmProviderFlavor; name: string; icon: string; description: string; tag?: string }) => (
      <button
        key={provider.id}
        onClick={() => {
        ...
        <div className="space-y-3">
          <div className="space-y-2">
            ...
            <div className="space-y-2 pt-2">
              <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Showcase</span>
              <div className="space-y-2">
                {showcaseOnly.map((provider) => (
                  <div
                    key={provider.name}
                    className="flex items-center gap-3 rounded-xl border border-border/70 bg-muted/20 px-3 py-3 opacity-70"
                  >
                    <ProviderIcon id={provider.icon} className="size-5 shrink-0 text-foreground" />
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="space-y-2 pt-2">
              ...
              {(showMoreProviders || isMoreProvider) ? (
                ...
                <button
                  ...
                  className="rounded-md px-0 text-xs text-muted-foreground transition-colors hover:text-foreground"
                >
                  Show more providers
                </button>
              )}
