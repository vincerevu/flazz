...
```bash
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /path/to/file.xlsx
```

**JSON output (for programmatic processing):**

```bash
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /path/to/file.xlsx --json
```

**Single-sheet mode (faster for targeted checks):**

```bash
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /path/to/file.xlsx --sheet Summary
```

**Summary mode (counts only, no per-cell detail):**

```bash
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /path/to/file.xlsx --summary
```
...
```bash
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\xlsx_unpack.py" /path/to/file.xlsx /tmp/xlsx_inspect/
```
...
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\libreoffice_recalc.py" --check

# Run recalculation (default timeout: 60s)
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\libreoffice_recalc.py" /path/to/input.xlsx /tmp/recalculated.xlsx

# For large or complex files, extend the timeout
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\libreoffice_recalc.py" /path/to/input.xlsx /tmp/recalculated.xlsx --timeout 120
```
...
```bash
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /tmp/recalculated.xlsx
```
...
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /path/to/output.xlsx

# Step 2: Dynamic check (if LibreOffice available)
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\libreoffice_recalc.py" /path/to/output.xlsx /tmp/recalculated.xlsx
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /tmp/recalculated.xlsx
```
...
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /path/to/edited.xlsx --json \
  | python -c "
import json, sys
...
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /path/to/user_file.xlsx --json > /tmp/validation_results.json

# Step 2: Unpack for manual inspection
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\xlsx_unpack.py" /path/to/user_file.xlsx /tmp/xlsx_inspect/

# Step 3: Dynamic recalculation
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\libreoffice_recalc.py" /path/to/user_file.xlsx /tmp/user_file_recalc.xlsx

# Step 4: Re-validate recalculated file
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\formula_check.py" /tmp/user_file_recalc.xlsx --json > /tmp/validation_after_recalc.json
