...
```bash
cp -r packages/core/src/application/assistant/skills/create-spreadsheets/templates/minimal_xlsx/ /tmp/xlsx_work/
```
...
```bash
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\shared_strings_builder.py" \
...
```bash
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\shared_strings_builder.py" --file strings.txt \
  > /tmp/xlsx_work/xl/sharedStrings.xml
```
...
```bash
"%FLAZZ_PYTHON%" "%FLAZZ_SKILL_ROOT%\create-spreadsheets\scripts\xlsx_pack.py" /tmp/xlsx_work/ /path/to/output.xlsx
```
